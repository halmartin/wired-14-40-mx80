...
add_multicast_group(std::shared_ptr<Socket> s, struct ip_mreqn* req)
{
...
#ifdef SOCKPROXY_TEST
    FILE* file = NULL;
    int new_fd = dup(g_csp->add_membership_fd);
    if (new_fd < 0) {
    ...
    if (fprintf(file, "GROUP_IP %s, VLAN %d", maddr, vid) < 0) {
...
drop_multicast_group(const AddrVlan& mm, std::shared_ptr<Socket> sock)
{
...
#ifdef SOCKPROXY_TEST
    FILE* file = NULL;
    int new_fd = dup(g_csp->del_membership_fd);
    if (new_fd < 0) {
        error("%s: dup failed, errno = %m", __func__);
        return -1;
    }
    ...
    if (fprintf(file, "GROUP_IP %s, VLAN %d", maddr, mm.vlan) < 0) {
        error("%s: Error writing to test handler = %m", __func__);
        fclose(file);
        return -1;
    }
