...
TEST_F(CSPFixture, Select)
{
    ...
    // First, let's just make sure we can timeout
    fd_set read_fds;
    FD_ZERO(&read_fds);
    FD_SET(fd, &read_fds);
    timeval tv = {
    ...
    timeval tv_before, tv_after, tv_diff;
    ASSERT_EQ_WITH_ERRNO(0, gettimeofday(&tv_before, NULL));
    ASSERT_EQ_WITH_ERRNO(0, CSP_select(fd + 1, &read_fds, NULL, NULL, &tv));
    ASSERT_EQ_WITH_ERRNO(0, gettimeofday(&tv_after, NULL));

    timersub(&tv_after, &tv_before, &tv_diff);
    ASSERT_EQ(0, tv_diff.tv_sec);
    ASSERT_GT(tv_diff.tv_usec, 245000);
    ASSERT_LT(tv_diff.tv_usec, 255000);
    ...
    EXPECT_GT(tv.tv_usec, 495000);
    EXPECT_LT(tv.tv_usec, 505000);
