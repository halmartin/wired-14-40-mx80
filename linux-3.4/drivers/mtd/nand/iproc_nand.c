 ...
 */
#ifdef CONFIG_MTD_NAND_IPROC_USE_JFFS2
#define NAND_MTD_WRITE_OOB_SEPARATELY
#endif /* CONFIG_MTD_NAND_IPROC_USE_JFFS2 */
...
static const struct of_device_id iproc_nand_dt_ids[] __initdata = {
    ...
    u32 *buf, u8 *oob)
{
    ...
    if (err_addr != 0) {
        ...
        printk(KERN_WARNING "%s: attempting to fix uncorrectable error at 0x%llx\n",
            DRV_NAME, (unsigned long long)err_addr);
            err_cnt = nand_verify_erased_page(mtd, chip, buf, addr);
...
#if defined(CONFIG_OF)
    ppdata.of_node = of_find_matching_node(NULL, iproc_nand_dt_ids);
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,2,0)
    mtd_device_parse_register(mtd, NULL, &ppdata, pd->parts, pd->nr_parts);
#else
