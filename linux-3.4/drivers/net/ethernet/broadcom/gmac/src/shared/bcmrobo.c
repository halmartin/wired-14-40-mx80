...
#define PAGE_ARL	0x04	/* "Address resolution lookup" */
...
#define REG_ARL_MULTI_PORT_CTL  0x0e /* Multiport control register */
#define REG_ARL_MULTIPORT_ADDR0 0x10 /* Multiport address 0 register */
#define REG_ARL_MULTIPORT_VEC0  0x18 /* Multiport vector 0 register */
#define REG_ARL_MULTIPORT_ADDR1 0x20 /* Multiport address 1 register */
#define REG_ARL_MULTIPORT_VEC1  0x28 /* Multiport vector 1 register */
/* Status Page Registers */
#define REG_STATUS_LINK	0x00	/* Link Status Summary */
#define REG_STATUS_SPEED	0x04	/* Datarate Summary */
#define REG_STATUS_DUPLEX	0x08	/* Duplex Status Summary */
...
bcm_robo_config_vlan(robo_info_t *robo, uint8 *mac_addr)
{
	...
	/* setup each vlan. max. 4095 vlans. */
	...
	for (vid = 0; vid <= VLAN_NUMVLANS; vid ++) {
		...
		/* Use the wildcard if an explicit one not defined */
		if (!ports) {
			ports = getvar(robo->vars, "vlan*ports");
			if (ports)
				add_static = false;
		}
...
vlan_setup:
		         ...
		         member);			/* vlan members */
		{
			...
			SPINWAIT((robo->ops->read_reg(robo, PAGE_VTBL, vtbla,
						      &val8, sizeof(val8)),
				  ((val8 & 0x80) != 0)), 100 /* usec */);
		}
	}
...
bcm_robo_enable_switch(robo_info_t *robo)
{
	...
	robo->ops->read_reg(robo, PAGE_ARL, REG_ARL_MULTI_PORT_CTL, &val16, sizeof(val16));
	val16 |= 0x5; /* "Compare MPORT_ETYPE0 and MPORT_ETYPE1, Forward based on MPORT_Vector 0/1 if matched" */
	robo->ops->write_reg(robo, PAGE_ARL, REG_ARL_MULTI_PORT_CTL, &val16, sizeof(val16));

	bzero(arl_addr, sizeof(arl_addr));
	arl_addr[7] = 0x88;
	arl_addr[6] = 0xcc; /* 0x88cc is the LLDP ethertype */
	robo->ops->write_reg(robo, PAGE_ARL, REG_ARL_MULTIPORT_ADDR0, arl_addr, sizeof(arl_addr));

	robo->ops->read_reg(robo, PAGE_ARL, REG_ARL_MULTIPORT_VEC0, &val32, sizeof(val32));
	val32 |= 0x1bf; /* all ports */
	robo->ops->write_reg(robo, PAGE_ARL, REG_ARL_MULTIPORT_VEC0, &val32, sizeof(val32));

	bzero(arl_addr, sizeof(arl_addr));
	arl_addr[7] = 0x88;
	arl_addr[6] = 0x8e; /* 0x888e is the EAPOL ethertype */
	robo->ops->write_reg(robo, PAGE_ARL, REG_ARL_MULTIPORT_ADDR1, arl_addr, sizeof(arl_addr));

	robo->ops->read_reg(robo, PAGE_ARL, REG_ARL_MULTIPORT_VEC1, &val32, sizeof(val32));
	val32 |= 0x1bf; /* all ports */
	robo->ops->write_reg(robo, PAGE_ARL, REG_ARL_MULTIPORT_VEC1, &val32, sizeof(val32));
...
robo_watchdog(robo_info_t *robo)
{
	...
	success = success && !robo->ops->read_reg(robo, PAGE_STATUS, REG_STATUS_LINK, &linksts, sizeof(linksts));
	success = success && !robo->ops->read_reg(robo, PAGE_STATUS, REG_STATUS_SPEED, &spdsts, sizeof(spdsts));
	success = success && !robo->ops->read_reg(robo, PAGE_STATUS, REG_STATUS_DUPLEX, &dupsts, sizeof(dupsts));
	if (!success)
		return; // leave values unchanged

	/* pdesc97 has 9 elements but port_states only 8 (9th port is CPU port). */
	for (i = 0; i < (sizeof(pdesc97) / sizeof(pdesc_t))-1; i++) {
		struct robo_port_state *port = &robo->port_states[i];
		port->updown = (linksts >> i) & 0x1;
		port->speed = (spdsts >> (2 * i)) & 0x3;
		port->full_duplex = (dupsts >> i) & 0x1;
	}
	return;
}
