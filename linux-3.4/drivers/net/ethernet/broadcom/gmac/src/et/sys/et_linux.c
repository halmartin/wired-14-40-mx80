...
et_start(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (et_ctf_pipeline_loopback(et)) {
...
#if !defined(CONFIG_MACH_MERAKI_PASS_BRCM_HDR)
		int bcm_hdr_size = 4;
		...
		__skb_push(skb, 4);
			...
			skb->data[2] = 0x00;
			skb->data[3] = 0x00;
#endif /* !defined(CONFIG_MACH_MERAKI_PASS_BRCM_HDR) */
	...
	defined(CONFIG_IPROC_FA2_CS_OFFLOAD_SMALL_PKT_WA))
		...
		if (skb->len < MIN_PACKET_SIZE) {
			unsigned int len = MIN_PACKET_SIZE - skb->len;
#ifdef CONFIG_IPROC_FA2_NAPT_BYPASS
			less68 = 1;
#endif /* CONFIG_IPROC_FA2_NAPT_BYPASS */
			ET_TRACE(("forcing skb->len (%d) to %d\n", skb->len, MIN_PACKET_SIZE));
			if (unlikely((skb->tail + len) > skb->end)) {
				...
				ET_TRACE(("et%d: NOT enough tailroom for BRCM minimum packet size.\n", et->etc->unit));
				skb = skb_copy_expand(sk_tmp, skb_headroom(sk_tmp), skb_tailroom(sk_tmp) + len, GFP_ATOMIC);
				PKTCFREE(et->osh, sk_tmp, TRUE);
				if (!skb) {
					ET_ERROR(("et%d: Failed to realloc tailroom for BRCM minimum packet size; NOT transmitting frame.\n", et->etc->unit));
					return 0;
				}
			}
			memset(skb_put(skb, len), 0, MIN_PACKET_SIZE);
		}
#if !defined(CONFIG_MACH_MERAKI_PASS_BRCM_HDR)
		__pskb_trim(skb, skb->len);
#endif /* !defined(CONFIG_MACH_MERAKI_PASS_BRCM_HDR */
...
bcm5301x_gmac_init_module(void)
{
	...
	if (!err) {
		/* load GMAC0 device */
		err = platform_driver_probe(&bcm5301x_gmac0_driver,
					    bcm5301x_gmac_probe);
		if (err) {
	...
	if (IPROC_NUM_GMACS>1) {
		...
		if (!err) {
			/* load GMAC1 device */
			err = platform_driver_probe(&bcm5301x_gmac1_driver,
						    bcm5301x_gmac_probe);
			if (err) {
				ET_ERROR(("ERROR module_init, could not iproc_platform_driver_register\n"));
				ET_ERROR(("Error Code = 0x%08x\n", err));
	...
	if (IPROC_NUM_GMACS>2) {
		...
		if (!err) {
			/* load GMAC2 device */
			err = platform_driver_probe(&bcm5301x_gmac2_driver,
						    bcm5301x_gmac_probe);
			if (err) {
				ET_ERROR(("ERROR module_init, could not iproc_platform_driver_register\n"));
				ET_ERROR(("Error Code = 0x%08x\n", err));
	...
	if (IPROC_NUM_GMACS>3) {
		...
		if (!err) {
			/* load GMAC3 device */
			err = platform_driver_probe(&bcm5301x_gmac3_driver,
						    bcm5301x_gmac_probe);
			if (err) {
				ET_ERROR(("ERROR module_init, could not iproc_platform_driver_register\n"));
				ET_ERROR(("Error Code = 0x%08x\n", err));
